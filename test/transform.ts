...
class TransformHandler {
  ...
  private async fetchFieldFromDb(platformName) {
    ...
    const platformsettingsObj:PlatformSettings = await prisma.platformSettings.findUnique({ where: { platformId: platformName }});
    ...
    const activities = await prisma.workspacePlatformSchemaMappings.findMany({
      where: {platformSettingsId: platformsettingsObj.id, schema: {type:"Activity"}, platformSchema : {type:"Activity"}},
      include :{
        schema: true,
        platformSchema: true
      }
    ...
    const comments = await prisma.workspacePlatformSchemaMappings.findMany({
      where: {platformSettingsId: platformsettingsObj.id, schema: {type:"Comments"}, platformSchema : {type:"Comments"}},
      include :{
        schema: true,
        platformSchema: true
      }
