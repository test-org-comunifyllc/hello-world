...
import { Discussion, Comments, Members } from '@/modules/integration/interface/integration.interface'
...
class VanilaForumService {
    ...
    public async getInitialDatas(): Promise<Comments[]> {
        try {
            // const comments = await this.getComments();
            const transformedComments = await this.transformWholeData(comments, 'Comments')
            const data = transformedComments.map(async (item) => {
                const platformMemberId = (item.platformMemberId).toString();
                const member = await prisma.member.findFirst({
                    where: { platformId: this.platformId, platformMemberId: platformMemberId }
                });
                let communifyId = uuidv4();
                return {
                    ...item,
                    id: communifyId,
                    ...
                    displayValue: `${item.name} post a comment`,
                    memberId: member.id
                    // memberid
                }
            })

            const commentData = await Promise.all<[]>(data);
    ...
    public async getInitialData(): Promise<{}> {
        ...
        const [users, activities, comments] = await Promise.all<any>([this.getUsers(), this.getActivity(), this.getComments()]);
        const transformedUsers = this.transformWholeData(users, 'Member');
        const transformedActivities = this.transformWholeData(discussion, 'Activity');
        const transformedComments = await this.transformWholeData(comments, 'Comments')
        const userData = transformedUsers.map((item) => {
            const id = uuidv4();
            if (item.organization && (item.organization).length > 50) {
                delete item.organization; // organization length in db need to change
            };
        ...
        if (transformedActivities) {
            const promiseActivityData = transformedActivities.map(async (item) => {
                ...
                return {
                    ...
                    comunifyMemberId: member.id,
                    platformId: this.platformId,
                    displayValue: `${item.name} post a discussion`,
                    platformMemberId: platformMemberId,
                    memberId: member.id
                    // memberid
                }
        ...
        try {
            ...
            for (let i = 0; value; i++) {
                const { data } = await axios.get<[]>(
                    `${this.vanilaBaseUrl}/users?expand=extended&limit=500&page=${page}`,
                    {
    ...
    public async getComments(): Promise<Comments[]> {
        try {
            let page: number = 1;
            let discussionData: DiscussionType[] = [];
            let value: boolean = true;
            let comments: CommentType[] = [];
            let mergedComments: Comments[] = [];
            const yesterday: Date = new Date(new Date().getTime() - (24 * 60 * 60 * 1000));
            const filteredDate: string = '2022-06-20T04:54:13.116Z ' //yesterday.toISOString();
            for (let i = 0; value; i++) {
            ...
            if (mergedDiscussionList.length) {
                const discussionId = mergedDiscussionList.map((discussion: Discussion) => {
